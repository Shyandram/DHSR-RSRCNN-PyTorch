...
from model import DHSRnet_griddhnet
...
def load_network(upscale_factor, device):
    SRDH = DHSRnet_griddhnet(upscale_factor=upscale_factor).to(device)
    SRDH.apply(weight_init)
    return SRDH

def load_pretrain_network(cfg, device):
    SRDH = DHSRnet_griddhnet().to(device)
...
def main(cfg):
    ...
    for epoch in range(cfg.epochs):
        ...
        for step, (ori_image, haze_image, haze_img_lr, ori_image_lr) in enumerate(trainloader):
            ...
            smooth_loss = torch.nn.functional.smooth_l1_loss(dh_image, ori_image_lr)
            sr_smooth_loss = torch.nn.functional.smooth_l1_loss(sr_image, ori_image)
            # perceptual_loss = vggloss(sr_image, ori_image)
            loss = smooth_loss + sr_smooth_loss#+ 0.04*perceptual_loss
            ...
            summary.add_scalar('smooth_loss', smooth_loss.item(), count)
            summary.add_scalar('sr_smooth_loss', sr_smooth_loss.item(), count)
            ...
            trainloader.set_postfix_str('lr: {:.2f}  | smooth_loss: {:.2f} / sr_smooth_loss: {:.2f}'.format(optimizer.param_groups[0]['lr'], smooth_loss.item(), sr_smooth_loss.item()))
