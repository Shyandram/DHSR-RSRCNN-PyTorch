...
from model import AODnet, SRCNN
...
def main(cfg):
    ...
    for epoch in range(cfg.epochs):
        ...
        for step, (ori_image, haze_image) in enumerate(trainloader):
            ...
            if step % cfg.print_gap == 0:
                ...
                summary.add_image('Origin_Images', make_grid(ori_image[:4].data, normalize=True, scale_each=True),
                                  count)
            tqdm.set_description_str('Epoch: {}/{}  |  Step: {}/{}  '.format(epoch + 1, cfg.epochs, step + 1, train_number))
            tqdm.set_postfix_str('lr: {:.2f}  | Loss: {:.2f}'.format(optimizer.param_groups[0]['lr'], loss.item()))
        ...
        for step, (ori_image, haze_image) in enumerate(valloader):
